...
const LevelMap = () => {
  ...
  const levels = [
    { level: 1, name: 'Land', color: 'bg-green-500', objective: 'Navigate through the treacherous terrain and find the hidden path.' },
    { level: 2, name: 'Water', color: 'bg-blue-500', objective: 'Dive deep into the ocean and discover the sunken treasure.' },
    { level: 3, name: 'Final', color: 'bg-purple-500', objective: 'Conquer the ultimate challenge and claim your victory!' },
  ];
...
return (
    <div className="relative w-full flex flex-col justify-center items-center overflow-hidden p-4">
            ...
            {levels.map((level, index) => (
                <motion.div
                    ...
                    className={`absolute top-1/2 transform -translate-y-1/2 w-24 h-24 rounded-full ${level.color} shadow-lg 
                                            ...
                                            transition-all duration-300 ease-in-out z-10 ${index==2 && "size-36"}`}
        ...
        <button
            className="mt-8 px-6 py-3 bg-blue-500 text-white rounded-full text-lg font-semibold hover:bg-blue-600 transition-colors"
            onClick={handleProgressOrRestart}
        >
            { currentLevel==0 ? "start" :  currentLevel === levels.length ? 'Restart' : 'Progress to Next Level'}
        </button>
    </div>
);
};
